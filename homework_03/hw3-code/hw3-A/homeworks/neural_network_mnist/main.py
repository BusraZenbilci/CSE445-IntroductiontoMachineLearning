...
def plot_and_save_graphs(results, filename_prefix):
    ...
    for batch_size in unique_batch_sizes:
        ...
        output_directory = "hw3_written"
        os.makedirs(output_directory, exist_ok=True)

        plt.savefig(f"{output_directory}/{filename_prefix}_batch{batch_size}.png")
        plt.close()
...
def main():
    ...
    for lr, batch_size, [model_name, model] in combos:
        ...
        print(
            f"{count}: {model_name_} accuracy:{accuracy} train loss:{history[-1]} test_loss:{test_loss} param_size:{count_parameters(model)}")
        count += 1
        result[model_name_] = {}
